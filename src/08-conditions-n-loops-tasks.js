...
*        width=20    
* 
* NOTE: Please use canvas coordinate space (https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API/Tutorial/Drawing_shapes#The_grid), it differs from Cartesian coordinate system.
* 
* @param {object} rect1
* @param {object} rect2
* @return {bool}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  let out = ((isStartIncluded)?'[':'(').concat((a < b)?a + ', ' + b : b +', ' + a).concat((isEndIncluded)?']':')');
  return out;
}
...
let ind = Math.max(str.lastIndexOf('['), str.lastIndexOf('{'), str.lastIndexOf('('),str.lastIndexOf('<'));
let nextChar = (str.charAt(ind) === '(') ? ')' : String.fromCharCode(str.charCodeAt(ind)+2);
...
function evaluateTicTacToePosition(position) {
  let win;
 let check = position.some(function(a, i, arr){    
    win = arr[i][0];
